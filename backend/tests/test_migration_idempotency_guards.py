...
def test_taxonomy_structural_migration_guards_column_renames() -> None:
    source = inspect.getsource(database._ensure_generated_question_schema)
    assert "RENAME COLUMN pattern_slug TO algorithm_slug" in source
    assert "column_name = 'pattern_slug'" in source
    assert "DROP COLUMN pattern_slug" in source
    assert "skill_misconception_catalog_skill_slug_misconception_tag_key" in source


def test_taxonomy_remap_only_touches_legacy_slugs() -> None:
    source = inspect.getsource(database._apply_taxonomy_remap_migration)
    assert "WHERE algorithm_slug = ANY($1::text[])" in source
...
def test_taxonomy_remap_drops_legacy_tables_after_remap() -> None:
    ...
    remap_index = source.index("UPDATE answer a")
